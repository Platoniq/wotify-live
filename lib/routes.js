...
app.get("/space:space", utils.setDefaults, function(req, res){
  ...
  res.render('space', {space: req.params.space, logo: config.logo});
});


app.get("/space:space/notes", utils.setDefaults, function(req, res){
  res.locals.meta.title = `${title} Steps Notes`;
  res.locals.bodyClass = "notes";
  models.Slide.findOne({space: req.params.space}, function(err, slide) {
    var chapters = _.uniq(
      _.union(config.defaultChapters, slide.chapters), false, function(item, key, a) { return item.id; });
    res.render('notes', {space: req.params.space, chapters});
  });
});
...
app.get("/notes", utils.setDefaults, function(req,res){
  ...
  res.render('allnotes', {hashtags: config.hashtags});
});
