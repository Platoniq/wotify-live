...
module.exports = function(io, client) {
  ...
  module.sendSlides = function(space, type, callback, to_client) {
    ...
    models.Slide
      .findOne({space:space}, function(err, slide){
        ...
        if(type === 'notes') {
          note_filter.type = 'note';
        } else if(slide.show !== 'all' && type !== 'all') {
          note_filter.type = slide.show;
        }
        // Rest of the filters only if type parameter is strict filtered
        if(type === 'filtered' && note_filter.type !== 'api') {
          var c = slide.chapter;
          if(c && c !== '_all_') {
            if(c == '_active_') {
        ...
        console.log('FILTER type[', type, '] mongo filter', note_filter);
