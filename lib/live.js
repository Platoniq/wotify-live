...
module.exports = function(app, server) {
  ...
  io.on('connection', function(socket) {
    ...
    if(socket.request.headers.referer.indexOf('/admin') != -1) {
    ...
    .exec(function(err, groups){
      ...
      groups.forEach(function(s){
        console.log('Initializing Group', s.step);
        // Send to this client only
    ...
    socket.on('step change', function(obj){
      ...
      Step.findOne({step:obj.step}, function(err, s){
        ...
        // Group defaults to the same step
        if(!s.group) {
          s.group = s.step;
        }
        s.save(function(err){
    ...
    var resendAllFilteredSlides = function(step) {
      _.each(timeouts, function(steps, client){
        _.each(steps, function(timeout, sKey) {
          // Send everybody, not to admin (current user)
          if(step === sKey && client !== clientId) {
            console.log('Resending client %s, Step %d', client, sKey);
      ...
      Slide.findOne({step:obj.step}, function(err, slide) {
        ...
        if(obj && obj.show) {
          ...
          slide.save(function(err, slide){
            ...
            // Send all to this admin
            timeoutSlides(slide.step, false);
          ...
          User.findOne({userId:add.userId}, function(err, user) {
            ...
            if(i > -1) {
              ...
              ids.sort();
              var last = parseInt(ids[ids.length-1], 10) || 0;
              console.log('Last Id', last);
              add.id = (last + 1) + '-n';
              slide.slides.push(add);
            }
            console.log('Saving slide', i, slide);
            slide.save(function(err, slide){
              if(err) return liveAdmin.sendError(err, true);
    ...
    socket.on('step panic', function(step, panic){
      ...
      Step.findOne({step:step}, function(err, s){
        ...
        s.save(function(err){
          ...
          io.emit('step init', s);
          if(s.panic) {
            liveAdmin.sendError('Panic request on Step ' + s.step +'!');
            liveAdmin.sendError('Help is on the way!', clientId);
          }
          else {
            liveAdmin.sendSuccess('Panic dismissed on Step ' + s.step);
            liveAdmin.sendError('Okay. Everything under control?', clientId);
          }
        });
      });
    });
